...
public class MoveBackAction extends AbstractWidgetAction {
  public static final String NAME = "MoveBack";
  private WidgetHolder widgetHolder;

  public MoveBackAction(WidgetHolder widgetHolder) {
    super(NAME, widgetHolder.getController());
    this.widgetHolder = widgetHolder;
  }

  public MoveBackAction(final JRuntimeDesignerController controller) {
  ...
  public void doAction(ActionEvent e) throws Exception {
    if (widgetHolder != null) {
      ...
      if (layer > 0) {
        widgetHolder.setLayer(0);
        panel.setLayer(widgetHolder, 0);
      }
    }
    ...
    if ((widgetHolder == null || widgetHolder.isSelected()) && selectionManager.hasSelection()) {
      for(IWidgetHolder selectedWidget : selectionManager.getSelectedWidgets()) {
        if (widgetHolder != null && selectedWidget == widgetHolder) {
          continue;
        }
        int layer = selectedWidget.getLayer();
        if (layer > 0) {
          selectedWidget.setLayer(0);
          if (selectedWidget instanceof JComponent) {
            panel.setLayer((JComponent) selectedWidget, 0);
          }
        }
      }
    }
