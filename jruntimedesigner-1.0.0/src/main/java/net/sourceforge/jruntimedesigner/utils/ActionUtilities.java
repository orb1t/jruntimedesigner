...
public class ActionUtilities {
  ...
  public static void actionConfigure(Action a, ResourceBundle res, String internalName) {
    ...
    try {
      ...
      try {
        ...
        if (url != null)
          myIcon = new ImageIcon(Toolkit.getDefaultToolkit().getImage(url));
      } catch (Exception e) {
      }
      if ((myIcon != null) && (myIcon.getIconHeight() > 0) && (myIcon.getIconWidth() > 0))
        a.putValue(Action.SMALL_ICON, myIcon);
    } catch (MissingResourceException ex) {
    }
    try {
   ...
   * @param a  the action
   */
  public final static void activateAcceleratorKeyOnButton(JButton btn, Action a)
  {
   ...
   * @param a  the action
   */
  public final static void activateAcceleratorKeyOnComponent(JComponent comp, Action a)
  {
    KeyStroke ks = (KeyStroke)a.getValue(Action.ACCELERATOR_KEY);
    if (ks != null) {
